...
Param(
    ...
    [string]$catalogRelativePath, # => App Catalog relative url "sites/apps/AppCatalog"
...
function GetRequest ($apiUrl, $webSession) {
    return Invoke-WebRequest -Uri $apiUrl -Method Get -WebSession $webSession -UseBasicParsing
    Invoke-WebRequest -Uri https://httpbin.org/headers -UseBasicParsing
}

# Post Web Request
function PostRequest($apiUrl, $webSession, $body) {
    return Invoke-WebRequest -Uri $apiUrl -Body $body -Method Post -WebSession $webSession -UseBasicParsing
}
...
# Read the xml
$xmlBody = Get-Content ($currentLocation + $releaseFolder + "\DeploySPFxToAppCatalogRequestBody.xml") -Encoding UTF8
